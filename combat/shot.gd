	...
	if not hit.is_empty():
		...
		if ship != null:
			...
			if ship.faction != faction:
				...
				# A SHELL IS WORTH SOMETHING (v0.158.0). Gunfire — and only
				...
				# numbers are exactly what they were. Inert on any unarmed hull,
				# so the whole lever is dive-scoped by construction.
				ship.net_damage_cell(cell, damage,
					Tunables.get_num("dive_shell_worth"))
