...
func _tick_dive(delta: float) -> void:
	...
	if is_instance_valid(local_ship):
		...
		# Lead Keel: multiplies `dive_rate_max`, the speed the DOWN stick asks for
		# under the rate controller (review §3.2) — applied where the rate is
		# stamped, a few lines below.
		...
		# ...times Lead Keel (`dive_rate_mult`, stamped above): the card scales the
		# speed the DOWN stick asks for, never the climb.
		local_ship.dive_rate_max = sink_v * local_ship.dive_rate_mult
...
func _dive_weather(delta: float) -> void:
	...
	for sid in _dive_surged:
		...
		if hull != null and is_instance_valid(hull):
			hull.extra_wind = dive_weather_at(hull.global_position, sid)
