...
## Lattice regions already generated by IslandGen (Vector2i index -> true) —
## the lazy world-generation ledger (see IslandGen.ensure_generated). Owned
## here so it lives and dies with the resident data it describes.
var gen_regions := {}
...
## Re-write every RECORDED edit onto the resident grid. Called after a lazy
## region generation (IslandGen.ensure_generated): a loaded save's diffs — or a
## late-join client's — may predate the region, and the fresh island paint must
## never clobber a player's dig/place. Cheap at typical edit counts (hundreds);
## if edits ever number tens of thousands, index them by region (seam).
func reapply_all_edits() -> void:
	for cell in _edits:
		set_cell(cell, int(_edits[cell]))
